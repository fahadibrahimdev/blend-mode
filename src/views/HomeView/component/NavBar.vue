...
<template>
  <div class="fixed top-0 left-1/2 transform -translate-x-1/2 pt-3 sm:pt-5 z-30 w-[95%] sm:w-[90%] mx-auto">
    <nav class="nav-bar bg-white rounded-6xl px-3 md:px-0 py-3 lg:py-3 xl:py-4 z-50 w-full">
      <div class="container flex justify-between items-center">
        <!-- Brand Logo -->
        <div class="brand-logo w-18 md:w-24 2xl:w-32">
          <RouterLink to="/">
        ...
        <span
            class="icon-hamburger text-primary text-xl"
            @click="$emit('toggleMenuSidebar', true)"
        />
        </div>
        <div class="hidden md:flex items-center md:gap-10 xl:gap-20 3xl:gap-28">
          <ul class="nav-links md:space-x-14 xl:space-x-24 3xl:space-x-38 md:flex lg:text-xl xl:text-2xl 3xl:text-3xl text-primary">
            <template v-for="(navLink, i) in menuNavLinks" :key="i">
              <li
                  class="cursor-pointer"
                  :class="{ 'font-bold text-accent': currentSection === navLink.sectionId }"
